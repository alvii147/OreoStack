...
if __name__ == '__main__':
    skipdirs = ['.venv', '.git', 'img']
    for subdir, dirs, files in os.walk('.'):
        if not any([subdir.startswith(f'./{d}') for d in skipdirs]) and 'index.md' in files:
            with open(f'{subdir}/index.md', 'r') as mdfile:
                mddata = mdfile.read()

            htmldata = markdown.markdown(mddata)
            with open(f'{subdir}/index.html', 'w') as htmlfile:
